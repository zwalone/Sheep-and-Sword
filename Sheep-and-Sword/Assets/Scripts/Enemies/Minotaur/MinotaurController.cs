...
{
    // Animations:
    private MinotaurView view;
    public bool IsHurting { get; private set; }
    public bool IsDead { get; private set; }

    // Movement:
    private MinotaurModel model;
    ...
    private void Awake()
    {
        view = GetComponent<MinotaurView>();
        model = GetComponent<MinotaurModel>();
    ...
    {
        // Change minotaur's position:
        rd2D.MovePosition(rd2D.position + new Vector2(model.Speed, 0) * Time.fixedDeltaTime);

        // Check if there is a wall or player in front of minotaur:
        ChangeMoveDirection();
    }
    ...
    {
        // Check if player is no longer in front of minotaur:
    ...
    IEnumerator Attack()
    {
        if (!IsDead)
        {
            ...
            // Let the minotaur move faster:
            model.Speed *= AttackSpeed;

            // Make a sound of attacking:
            Invoke(nameof(SoundAttack), 0.2f);
    ...
    private void ChangeMoveDirection(bool behind = false)
    {
        ...
        // If there is a wall in front of minotaur, turn around:
    ...
    // Get minotaur's health points' values:
