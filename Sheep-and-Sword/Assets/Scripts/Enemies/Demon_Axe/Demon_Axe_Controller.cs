...
{
    // Animations:
    private Demon_Axe_View view;
    public bool IsHurting { get; private set; }
    public bool IsDead { get; private set; }

    // Movement:
    private Demon_Axe_Model model;
    ...
    private void Awake()
    {
        view = GetComponent<Demon_Axe_View>();
        model = GetComponent<Demon_Axe_Model>();
    ...
    {
        // Change demon_axe's position:
        rd2D.MovePosition(rd2D.position + new Vector2(model.Speed, 0) * Time.fixedDeltaTime);

        // Check if there is a wall or player in front of demon_axe:
        ChangeMoveDirection();
    }
    ...
    {
        // Check if living player showed up in front of demon_axe:
    ...
    {
        // Check if player is no longer in front of demon_axe:
    ...
    {
        // If demon_axe is dead, do nothing:
        if (IsDead) return;
        ...
        // If there is no ground in front of demon_axe, turn around:
        ...
        // If there is a wall in front of demon_axe, turn around:
    ...
    {
        // If demon_axe is dead or just received damage, do nothing:
        if (IsDead || !canHurt) return;

        // Check if player is behind the demon_axe and turn around:
    ...
    // Get demon_axe's health points' values:
